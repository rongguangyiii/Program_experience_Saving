...
SolidSolverLSG::SolidSolverLSG(const std::shared_ptr<GridBase> g, size_t nx, size_t ny) : residual_(0.0), grid_(g), caltime_(0),
...
void SolidSolverLSG::solve()
{
	GlobalData::createFolder("../../resultLSG");
	if (useRef_) {
...
void SolidSolverLSG::initialize() {
...
void SolidSolverLSG::initialize_ref() {
...
void SolidSolverLSG::timeAdvance()
{
	size_t timestep = 0;
	residual_ = 1.0;
...
void SolidSolverLSG::computeRHSTrans()
{
	calPartialDer();
	//calPartialDer_ref();
...
void SolidSolverLSG::solveEulerTrans()
{
	size_t index = 0;
...
void SolidSolverLSG::computeResidual()
{
	residual_ = 0.0;
...
void SolidSolverLSG::extrapBoundary()
{
...
void SolidSolverLSG::writeTecplotFile(size_t timestep) const {
	//if (timestep % GlobalData::ctrlout != 0)
	//	return;
	const auto& x = grid_->getXCoord();
	const auto& y = grid_->getYCoord();
	std::ofstream outFile("../../resultLSG/solution_" + std::to_string(timestep) + ".dat");
...
//Fu,Fv,Gu,Gv-------------------------------------------------------------------------------------------------------
double SolidSolverLSG::Fu(size_t i, size_t j, const CoefTrans& trans)
{
...
double SolidSolverLSG::Fv(size_t i, size_t j, const CoefTrans& trans)
{
...
double SolidSolverLSG::Gu(size_t i, size_t j, const CoefTrans& trans)
{
	double ksix = trans.ksi_x();
	double ksiy = trans.ksi_y();
	double J = trans.jacob();
...
double SolidSolverLSG::Gv(size_t i, size_t j, const CoefTrans& trans)
{
	double etax = trans.eta_x();
	double etay = trans.eta_y();
	double J = trans.jacob();
...
//dFu_dksi,dFv_dksi------------------------------------------------------------------------------------
double SolidSolverLSG::dFu_dksi(size_t i, size_t j)
{
	...
	double result = (Fu(i, j, trans) - Fu(i - 1, j, trans)) / 2;
	DFuDksi_[i][j] = result;
	return result;
}
double SolidSolverLSG::dFv_deta(size_t i, size_t j)
{
	size_t index = i * ny_ + j;
	const auto& trans = grid_->getCoefTransVec()[index];
	double result = (Fv(i, j, trans) - Fv(i, j - 1, trans)) / 2;
	DFvDeta_[i][j] = result;
	return result;
}
double SolidSolverLSG::dGu_dksi(size_t i, size_t j)
{
	size_t index = i * ny_ + j;
	const auto& trans = grid_->getCoefTransVec()[index];
	double result = (Gu(i, j, trans) - Gu(i - 1, j, trans)) / 2;
	return result;
}
double SolidSolverLSG::dGv_deta(size_t i, size_t j)
{
	size_t index = i * ny_ + j;
	const auto& trans = grid_->getCoefTransVec()[index];
	double result = (Gv(i, j, trans) - Gv(i, j - 1, trans)) / 2;
	return result;
}
//LSGrad---------------------------------------------------------------------------
void  SolidSolverLSG::calPartialDer()
{
...
void  SolidSolverLSG::calPartialDer_ref()
{
	std::vector<std::string> dirname{ "u","v" };
	const auto& x = grid_->getXCoord();
	const auto& y = grid_->getYCoord();
...
std::vector<LSGradCoord>  SolidSolverLSG::genLSGradCoord(size_t i, size_t j, std::string flag)
{
	const auto& x = grid_->getXCoord();
	const auto& y = grid_->getYCoord();
